...
# BatBar ⚡ v1.0
# Version compatible with Windows, macOS and Linux.
# A simple battery bar that shows the battery percentage and charging status.
# by simonquasar
...
class BatteryBar:
    ...
    def setup_mouse_bindings(self):
        if self.os_type == "darwin":
            ...
            self.battery_bar.bind("<MouseWheel>", self.on_mouse_wheel)
            self.battery_bar.bind("<Button-3>", self.on_mouse_click)  # Right click Windows/Linux
            self.root.bind("<MouseWheel>", self.on_mouse_wheel)
